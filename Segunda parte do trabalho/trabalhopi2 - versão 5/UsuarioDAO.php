<?php
    ...
    class UsuarioDAO{
        ...
        public function atualizar(Usuario $usuario){
            $qAtualizar = "UPDATE usuario SET login_usuario=:login_usuario, senha_usuario=:senha_usuario, nome_usuario=:nome_usuario WHERE id_usuario=:id_usuario";            
            $pdo = PDOFactory::getConexao();
        ...
        // Busca o usuário pelo seu login
        public function buscarPorLogin($login_usuario)
        {
 		    $query = 'SELECT * FROM usuario WHERE login_usuario=:login_usuario';		
            $pdo = PDOFactory::getConexao(); 
		    $comando = $pdo->prepare($query);
		    $comando->bindParam ('login_usuario', $login_usuario);
		    $comando->execute();
		    $row = $comando->fetch(PDO::FETCH_OBJ);
		    return new Usuario($row->id_usuario,$row->login_usuario,$row->senha_usuario,$row->nome_usuario);           
        }
    }
?>